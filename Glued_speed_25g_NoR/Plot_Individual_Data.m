...
TD_files=dir('*rec_noRep*ls*TD*');
ASD_files=dir('*rec_noRep*ls*ASD*');
Gfig=figure; hold on; 
...
%         plot_EbarsNew(ls(j).phat, ls(j).ci,shapes{i},'r');
...
%         plot_EbarsNew(ls(j).phat, ls(j).ci,shapes{i},'b');
...
print -dtiff Gamma_Moments_Indiv_NoRep;

figure(Gfig); maximize;
print -dtiff Gamma_Plane_Indiv_NoRep;

figure(pdf_fig); maximize;
print -dtiff Gamma_PDF_Indiv_noRep;
